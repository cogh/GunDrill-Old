...
switch state {
    case "right":
		if obj_timer.click {
			...
			target_x = lerp(range.bbox_left,range.bbox_right-sprite_get_width(sprite_index),obj_timer.time);
			hsp = round(target_x-x);
		}
        break;
    case "left":
		if obj_timer.click {
			...
			target_x = lerp(range.bbox_right-sprite_get_width(sprite_index),range.bbox_left,obj_timer.time);
			hsp = round(target_x-x);
		}
        break;
}
